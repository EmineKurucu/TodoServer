...
const getDayRange = async(req, res , next) => {
...
exports.getWeeklyReport = async (req, res, next) => {
    try {
        ...
        if (!date) {
            return res.status(400).json({error: "Tarih zrounludur"});
        ...
        const todos = snapshot.doc.map(doc => ({
            id : doc.id,
            ...doc.data()
        }));
        ...
        // kuallnıcının haftalık notunu çek
        const noteDoc = await db.collection("weeklyNotes").doc(userID).get();
        const noteData = noteDoc.exists ? noteDoc.data() : {};
        const weekKey = start.toISOString().split("T")[0];
        const userNote = noteData[weekKey] || null;
        ...
        res.status(200).json({
            ...
            completedCount,
            notCompletedCount,
            completitionRate : total ? ((completedCount / 100) * 100).toFixed(2) : "0.00",
